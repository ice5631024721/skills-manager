...
- **Upstream deleted detection** — When a skill's directory is removed from its upstream repo, the card shows a red **Upstream deleted** badge, its updates are held back, and the local copy is kept untouched.
...
- **MCP servers, managed** — Keep MCP server definitions in one library and deploy them from there: each card shows a dot per supported agent, and clicking a dot writes or removes that entry in the agent's own config file (OpenCode and DeepSeek Harness today, through surgical edits that leave the rest of the file untouched). An upstream update on an npm / npx / PyPI-uvx / Git source raises an **Update available** badge and only applies after a confirm dialog listing the exact commands; a definition can also be checked with a one-shot liveness probe. Entries you added by hand inside an agent show up as foreign and can be taken over into the library with one click. Scope, honestly: only those two agents' global config files — project-level MCP files are left to git.
...
- **In-app updates** — The app notifies you when a new version is out; updating stays a decision you make. Nothing downloads or installs on its own — on macOS and Windows you then click **Install Update** in Settings, and installing and restarting each take a click.
