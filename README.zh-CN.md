...
- **上游已删除检测** — 当 Git 技能在上游仓库中的目录被删除时，技能卡片会显示红色「上游已删除」徽章，该技能的更新将被搁置，本地副本原样保留。
- **文档预览** — 直接在应用内查看 `SKILL.md` / `README.md`。
- **MCP 服务托管** — 把 MCP 服务器定义集中放在托管定义库里，从这里部署出去：每张卡片按 agent 显示一个圆点，点击即在 agent 自己的配置文件里写入或移除这条条目（目前支持 OpenCode 与 DeepSeek Harness，采用外科手术式改写，文件其余部分原样保留）。npm / npx / PyPI-uvx / Git 上游源有新版时会出现 **可升级** 徽章，只有在你看过确认框里逐条列出的命令后才会真的执行；定义也可以做一次一次性探活。你直接在 agent 配置里添加的条目会以外来条目显示，一键即可接管进定义库。范围说清楚：只动这两个 agent 的全局配置文件，项目级 MCP 配置留给 git 仓库管。
...
- **应用内更新** — 有新版本时应用会提醒你，要不要更新由你决定。不会自行下载或安装：在 macOS 和 Windows 上，你需要自己在设置里点击 **安装更新**，安装和重启各需一次点击。
